...
struct dleaf { //leaf of deployment tree
	l8 Sid;
	l8 Mid;
	l8 Rtt;
};
...
void h1(byte *input, byte *dig)
{
        ...
        for (l=0;l<8;l++) {
                ...
                for (i=0;i<255;i++) {
                        if (k>ilen-1) k=0;
                        a=s[i]; j=j^s[i]^input[k];
                        s[i]=s[j];
                        s[j]=a;
                        k++;
                }
                if (k>ilen-1) k=0;
                a=s[255]; j=j^s[255]^input[k];
                s[255]=s[j];
                s[j]=a;
        }
        for (i=0;i<20;i++)
...
void fl2r(byte ln[20], byte* lhashes, byte l, int index, byte* out) {
	...
	byte order;
	for (i=0;i<l;i++){
		order=(byte )index%2; printf("order %d  ", order);
...
byte meop (byte op, byte ad1, byte ad2, l8 *V) {
    byte aop,out;
    l8 lout, in1,in2;
    if (op==0) return 0; //stop execution
        aop=op>>2;
    out=op-(aop<<2);
    if (aop==1) lout = V[ad1]+V[ad2];
    else if (aop==2)  lout = V[ad1]-V[ad2];
    else if (aop==3)  lout = V[ad2]-V[ad1];
    else if (aop==4)  lout = V[ad1];
    else if (aop==5)  lout = !V[ad1];
    else if (aop==6)  lout = ~V[ad1];

    else if (aop==7)  lout=V[ad1]&V[ad2];
    else if (aop==8)  lout=V[ad1]&(~V[ad2]);
    else if (aop==9)  lout=(~V[ad1])&V[ad2];
    else if (aop==10) lout=(~V[ad1])&(~V[ad2]);

    else if (aop==11) lout=V[ad1]|V[ad2];
    else if (aop==12) lout=V[ad1]|(~V[ad2]);
    else if (aop==13) lout=(~V[ad1])|V[ad2];
    else if (aop==14) lout=(~V[ad1])|(~V[ad2]);

    else if (aop==15) lout=V[ad1]&&V[ad2];
    else if (aop==16) lout=V[ad1]&&(!V[ad2]);
    else if (aop==17) lout=(!V[ad1])&&V[ad2];
    else if (aop==18) lout=(!V[ad1])&&(!V[ad2]);

    else if (aop==19) lout=V[ad1]||V[ad2];
    else if (aop==20) lout=V[ad1]||(!V[ad2]);
    else if (aop==21) lout=(!V[ad1])||V[ad2];
    else if (aop==22) lout=(!V[ad1])||(!V[ad2]);

    if (out==0) V[0]=lout;
    else if (out==1) V[24]=lout;
    else if (out==2) V[25]=lout;
    else if (out==3) V[26]=lout;
    return 1;
}
...
byte Flsr (byte pos, byte nu[20], byte mu[20]){
	...
	memcpy(&(M.ltmp1[41]),M.LAMBDA,20);
	h1(M.ltmp1,M.htmp1);
	if (memcmp(M.htmp1,mu,20)) {printf("problem in Flsr\n", pos ); return 1;}
	memcpy(&M.lambda[pos],nu,20); 
	return 0;
}
void Fllf (byte pos, void *l1, void* l2, void* l3) {
    fhleaf(l1,l2,l3,M.htmp1);	
	if (memcmp(M.htmp1,&(M.lambda[pos]),20)) { printf("Fllf %d \n", pos); return;}
	memcpy(&(M.V[16+pos*11]), l1,40);
	memcpy(&(M.V[21+pos*11]), l3,48);
	if (pos==0) memcpy(M.c, l2,24);
	return;
}
void Fupd(byte nrt[20], byte mu[20]) {
	if (M.V[16] != M.V[12]) {printf("two SIds \n"); return;}
	if (M.V[18] != M.V[27]) return;
	if (M.V[19] != M.V[38]) return;
	if (M.V[20] != M.V[49]) return;
	if ((M.V[14]+M.V[17]) < M.V[22]) {printf("time \n"); return;}
	M.V[21]=M.V[13];
	...
	h1(M.ltmp2,M.htmp2);
	if (memcmp(M.htmp2,mu,20)) { printf("type 2 cert fail \n"); return;}
...
node* cur;
	...
	while (cur != NULL)	{
		if (key < cur->key) {
			...
			if (get) {memcpy(dp,cur->data, 20); //printf("getting %d \n",key);
					 }
			else {memcpy(cur->data,dp, 20); //printf("updating %d \n",key);
				 } 
			return cur;
		}
	}
...
void buildmerkle(int lev, node* btroot) 	{
	...
	while (n=n>>1) {
		...
		for (i=0; i< n; i++) {
			l=getkey(2*i,j-1);r=getkey(2*i+1,j-1);
			put(l,btroot,tl,1);
			put(r,btroot,tr,1);
			hnode(tl,tr,0,tmp);
...
void updateleaf(leaf *plf, int index, node* root, l8 Val, l8 Tr, l8* V)	{
	...
	l2r(pold,iv,LEV,t20);
	if (memcmp(t20,croot,20) != 0) printf("WE HAVE PROBLEMS! (POS0) \n"); 
	if (memcmp(t20,M.root,20) != 0) printf("WE HAVE PROBLEMS! (POS0) mod \n"); 
	for (i=16; i<60; i++) V[i]=0;
	...
	if (tmp>0) {
		...
		l2r(p1,ivs,LEV,t20);
		if (memcmp(t20,croot,20) != 0) printf("WE HAVE PROBLEMS! (POS1) \n"); 
	...
	if (tmp>0) {
		...
		getvector(tmp-1, ivs, LEV,root);
		l2r(p1,ivs,LEV,t20);
		if (memcmp(t20,croot,20) != 0) printf("WE HAVE PROBLEMS! (POS2) \n"); 
		Fc1(p1,ivs,LEV,t20);//t20 is the MAC
		Flsr(2,p1,t20);
	...
	if (tmp>0) {
		...
		getvector(tmp-1, ivs, LEV,root);
		l2r(p1,ivs,LEV,t20);
		if (memcmp(t20,croot,20) != 0) printf("WE HAVE PROBLEMS! (POS3) \n"); 
		Fc1(p1,ivs,LEV,t20);//t20 is the MAC
		Flsr(3,p1,t20);
		Fllf(3,&plf[tmp-1].Sid, &plf[tmp-1].c, &plf[tmp-1].Sval);
	}
	if (plf[index].Sid==0) return;
	...
	put(getkey(0,LEV),root,croot,1);//get current root
	if (memcmp(t20,croot,20) != 0) printf("WE HAVE PROBLEMS! (after update) \n");
...
int chkret = 0;
unsigned char *token;
l8 input[32];l8 var,time; //time, last value to KILL
...
//decl
   ...
   byte tmplong[20*LEV]={0};
    ...
    Finit(sroot, droot);
	if(memcmp(M.sroot,sroot,20)) {printf("fix this"); return;}
